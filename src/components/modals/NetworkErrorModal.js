...
//----COMPONENT----//
const NetworkErrorModal = ({ isOpened, closeCallback }) => {
    //Data and State
                ...
                <View style={styles(scales).modal_card}>                    
                    ...
                    <View style={styles(scales).card_body}>
                        <View style={styles(scales).icon_container}>
                            <IcoMoonIcon name="sad" color={app_styles(scales).colors.text.grey} size={Math.round(scales.widthScale * 50)} />
                        </View>
                        <Spacer spaceHeight={16} />
                        <Text style={styles(scales).card_title}>{translator.translate(language, "Помилка підключення")}</Text>
                        <Spacer spaceHeight={8} />
                        <Text style={styles(scales).info}>{translator.translate(language, "Перевірте інтернет-з’єднання")}</Text>
                    </View>
                    {/* Close button */}
